...
/**
 * QuickWeds 5000-Concurrent-User Stress Test
 *
 * Simulates extreme traffic spikes with 5 ramp-up stages:
 *   Stage 1:   100 users  (warmup)
 *   Stage 2:   500 users  (moderate)
 *   Stage 3:  1500 users  (heavy)
 *   Stage 4:  3000 users  (extreme)
 *   Stage 5:  5000 users  (peak-5000)
 *
 * Usage:
 *   npm run test:stress:5000
 *   STRESS_WEDDING_ID=amanda-and-sayato-3a6409 npm run test:stress:5000
 *   BASE_URL=https://quickweds.site npm run test:stress:5000
 */
...
// ── Ramp-up stages up to 5000 users ──────────────────────────────────
...
const stages = [
  {
    ...
    budgets: { maxP95Ms: 2000, maxErrorRate: 0.05, minRps: 20 },
  },
  {
    name: 'moderate',
    concurrency: 500,
    ...
    budgets: { maxP95Ms: 4000, maxErrorRate: 0.10, minRps: 100 },
  },
  {
    name: 'heavy',
    concurrency: 1500,
    ...
    budgets: { maxP95Ms: 6000, maxErrorRate: 0.15, minRps: 200 },
  },
  {
    name: 'extreme',
    concurrency: 3000,
    durationMs: 25_000,
    endpoints: [...ENDPOINTS.pages, ...ENDPOINTS.wedding, ...ENDPOINTS.apiReads, ...ENDPOINTS.assets],
    budgets: { maxP95Ms: 10000, maxErrorRate: 0.20, minRps: 300 },
  },
  {
    name: 'peak-5000',
    concurrency: 5000,
    durationMs: 30_000,
    endpoints: [...ENDPOINTS.pages, ...ENDPOINTS.wedding, ...ENDPOINTS.apiReads, ...ENDPOINTS.assets],
    budgets: { maxP95Ms: 15000, maxErrorRate: 0.30, minRps: 500 },
  },
];
...
async function makeRequest(endpoint) {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'QuickWeds-StressTest/5000',
...
console.log('╔══════════════════════════════════════════════════════════════╗');
console.log('║         QuickWeds 5000-User Stress Test                     ║');
console.log('╚══════════════════════════════════════════════════════════════╝');
...
if (anyBudgetFailed) {
  console.log('\n❌ 5000-User stress test completed with budget violations.');
  process.exit(1);
} else {
  console.log('\n✅ 5000-User stress test passed — all stages within budget.');
}
