...
export const checkoutSchema = z.object({
  ...
  plan: z
    .enum(["account_pro", "planner_pro", "premium", "elite"])
    .default("planner_pro"),
});
...
export const rsvpSubmissionSchema = z
    ...
    attendees: z
      .array(
        z.object({
          ...
          meal: z.string().max(200).optional().default(""),
  ...
  .superRefine((value, context) => {
    if (value.childrenCount > value.numGuests)
      context.addIssue({
        code: "custom",
        path: ["childrenCount"],
        message: "Children are included in the total number attending.",
      });
    if (value.householdMembers.length > value.numGuests - 1)
      context.addIssue({
        code: "custom",
        path: ["householdMembers"],
        message: "Additional names cannot exceed the party size.",
      });
    if (value.attendees.length && value.attendees.length !== value.numGuests)
      context.addIssue({
        code: "custom",
        path: ["attendees"],
        message: "Add details for each person in the party.",
      });
    if (
      value.attendees.length &&
      value.attendees.filter((person) => person.kind === "child").length !==
        value.childrenCount
    )
      context.addIssue({
        code: "custom",
        path: ["childrenCount"],
        message: "The child count must match the people listed below.",
      });
  });
