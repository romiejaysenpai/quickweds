...
const DEFAULT_LAYOUT = {
  ...
  layout_data: {
    ...
    objects: [
      {
        id: "dance-floor-default",
        type: "dance_floor" as VenueObjectType,
        label: "Dance Floor",
        x: 50,
        y: 52,
        width: 28,
      ...
      {
        id: "entrance-default",
        type: "entrance" as VenueObjectType,
        label: "Entrance",
        x: 50,
        y: 96,
        width: 12,
...
const VENUE_OBJECT_PRESETS: Record<
  VenueObjectType,
...
function getPartySize(
  guest?: Pick<
    EnhancedRSVP,
    "num_guests" | "plus_one_allowed" | "plus_one_name" | "plus_one_rsvp_status"
  > | null,
) {
...
export default function SeatingChartBuilder({
  ...
  const lastTableDragPositionRef = useRef<Pick<
    Table,
    "position_x" | "position_y"
  > | null>(null);
  const lastObjectDragPositionRef = useRef<Pick<VenueObject, "x" | "y"> | null>(
    null,
  );
  ...
  const [seatFinderLoading, setSeatFinderLoading] = useState<
    "generate" | "send" | "resend" | null
  >(null);
  ...
  const [publicSeatFinderToken, setPublicSeatFinderToken] = useState(
    initialSeatFinderEnabled ? initialPublicSeatFinderToken || "" : "",
  );
  ...
  const assignedGuestIds = useMemo(
    () => new Set(assignments.map((assignment) => assignment.rsvp_id)),
    [assignments],
  );
  const seatsUsedByTable = useMemo(() => {
    ...
    assignmentsByTable.forEach((tableAssignments, tableId) => {
      counts.set(
        tableId,
        tableAssignments.reduce(
          (total, assignment) =>
            total + getPartySize(guestMap.get(assignment.rsvp_id)),
          0,
        ),
      );
    });
  ...
  const spreadDefaultPositions = useCallback((loadedTables: Table[]) => {
    return loadedTables.map((table, index) => {
      if (Number(table.position_x) > 0 || Number(table.position_y) > 0)
        return table;
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [guestsRes, tablesRes, assignmentsRes, layoutRes] =
        await Promise.all([
          supabase
            .from("rsvps")
            .select(
              "id, guest_name, guest_group, guest_email, rsvp_status, attendance, num_guests, table_assignment, plus_one_name, plus_one_allowed",
            )
      ...
      if (layoutRes.error) {
        ...
        const loadedLayoutData = (layoutRes.data.layout_data ||
          {}) as SeatingLayoutData;
        setLayout({
          wedding_id: weddingId,
          ...
          venue_height:
            layoutRes.data.venue_height || DEFAULT_LAYOUT.venue_height,
          grid_enabled:
            layoutRes.data.grid_enabled ?? DEFAULT_LAYOUT.grid_enabled,
          layout_data: {
            ...DEFAULT_LAYOUT.layout_data,
            ...loadedLayoutData,
            objects: Array.isArray(loadedLayoutData.objects)
              ? loadedLayoutData.objects
              : DEFAULT_LAYOUT.layout_data.objects,
          },
  ...
  const getSeatsUsed = useCallback(
    (tableId: string) => seatsUsedByTable.get(tableId) || 0,
    [seatsUsedByTable],
  );
  ...
  const syncRsvpTableAssignment = async (
    rsvpId: string,
  ...
  const saveLayout = async (nextLayout: SeatingLayout) => {
    ...
    if (error) {
      ...
      if (missingLayoutData) {
        ...
        if (!fallback.error) {
          console.warn(
            "Seating floor-plan objects require the latest supabase-power-features.sql layout_data migration.",
          );
          return;
        }
      }
  ...
  const updateLayout = (patch: Partial<SeatingLayout>) => {
    const nextLayout = {
      ...
      ...patch,
      venue_width: clamp(
        Number(patch.venue_width ?? layout.venue_width),
        30,
        240,
      ),
      venue_height: clamp(
        Number(patch.venue_height ?? layout.venue_height),
        30,
        240,
      ),
    };
  ...
  const deleteVenueObject = (objectId: string) => {
    updateLayoutData({
      objects: (layout.layout_data?.objects || []).filter(
  ...
  const openAddTableModal = () => {
    if (!hasPlannerPro && tables.length >= FREE_PLAN_LIMITS.seatingTables) {
      alert(
        `Free seating includes ${FREE_PLAN_LIMITS.seatingTables} tables. Upgrade to Planner Pro for the full seating chart, guest check-in, and seat-link sending.`,
      );
      return;
    }
    ...
    try {
      if (editingTableId) {
        ...
        if (tableFormData.capacity < currentSeatsUsed) {
          return alert(
            `This table already has ${currentSeatsUsed} assigned seats. Remove guests before reducing capacity below that number.`,
          );
        }

        const { data: session } = await supabase.auth.getSession();
        const response = await fetch("/api/seating/table", {
          method: "PATCH",
          headers: {
        ...
        setTables((current) =>
          current.map((table) => (table.id === editingTableId ? data : table)),
        );
      } else {
      ...
      const message =
        err instanceof Error ? err.message : "Unknown seating save error";
      const guidance =
        /permission denied|rls|row level security|could not find|blocked by database permissions/i.test(
          message,
        )
  ...
  const updateTablePosition = useCallback(
    async (tableId: string, position_x: number, position_y: number) => {
      try {
  ...
  const updateTableStyle = async (
    tableId: string,
    patch: NonNullable<Table["custom_style"]>,
  ) => {
    ...
    if (error) {
      setPositionSaveError(
        "Unable to save table style. Apply the latest seating migration.",
      );
    } else {
      setPositionSaveError("");
    }
  ...
  const deleteTable = async (tableId: string) => {
    if (
      !confirm("Are you sure? All assignments to this table will be removed.")
    )
      return;
    ...
    try {
      ...
      const response = await fetch("/api/seating/table", {
        ...
        body: JSON.stringify({ weddingId, tableId }),
      });
      const result = await response.json();
      if (!response.ok)
      ...
      const message =
        err instanceof Error ? err.message : "Unknown seating delete error";
  ...
  const assignGuest = async (guest: EnhancedRSVP, tableId: string) => {
    ...
    if (partySize > seatsRemaining) {
      return alert(
        `This table only has ${seatsRemaining} open seat${seatsRemaining === 1 ? "" : "s"}, but this party needs ${partySize}.`,
      );
    }

    try {
      const { data: session } = await supabase.auth.getSession();
      const response = await fetch("/api/seating/assign", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session.session?.access_token}`,
        },
        body: JSON.stringify({ weddingId, guestId: guest.id, tableId }),
      });
      if (!response.ok) throw new Error((await response.json()).error);
      await loadData();
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : "Unknown seating assignment error";
  ...
  const removeAssignment = async (assignment: Assignment) => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Unknown seat removal error";
      alert(`Failed to remove assignment: ${message}`);
    }
  };

  const callSeatFinderApi = async (
    path: string,
    ...
    const token = sessionData.session?.access_token;
    if (!token)
      throw new Error("Please sign in again before using QR Seat Finder.");
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok)
      throw new Error(data.error || "QR Seat Finder request failed.");
    return data;
  };

  const getClientSeatFinderUrl = useCallback(
    (token?: string | null, fallbackUrl?: string | null) => {
  ...
  useEffect(() => {
    ...
    setPublicSeatFinderUrl(
      getClientSeatFinderUrl(initialPublicSeatFinderToken),
    );
  }, [
    getClientSeatFinderUrl,
    initialPublicSeatFinderToken,
    initialSeatFinderEnabled,
  ]);
  ...
  const generateSeatLinks = async () => {
    ...
    setSeatFinderStatus("");
    try {
      const data = await callSeatFinderApi("/api/seating/generate-seat-links", {
        weddingId,
      });
      const token = data.publicSeatFinderToken || "";
      setPublicSeatFinderToken(token);
      setPublicSeatFinderUrl(
        getClientSeatFinderUrl(token, data.publicSeatFinderUrl),
      );
      ...
      setSeatFinderStatus(
        `Guest codes ready. ${data.generated || 0} new code${data.generated === 1 ? "" : "s"} generated.`,
      );
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Unable to generate guest codes.";
  ...
  const sendSeatLinks = async (resendAll = false) => {
    ...
    setSeatFinderStatus("");
    try {
      const data = await callSeatFinderApi("/api/seating/send-seat-links", {
        weddingId,
        resendAll,
      });
      setSeatFinderStatus(
        `Seat links sent: ${data.sent || 0}. No email: ${data.skippedNoEmail || 0}. Unassigned: ${data.skippedUnassigned || 0}.${data.emailErrorCount ? ` Email errors: ${data.emailErrorCount}.` : ""}`,
      );
      ...
      const message =
        err instanceof Error ? err.message : "Unable to send seat links.";
      setSeatFinderStatus(message);
      alert(message);
    } finally {
      setSeatFinderLoading(null);
  ...
  const getPublicSeatFinderOpenUrl = () => {
    const url =
      publicSeatFinderUrl || getClientSeatFinderUrl(publicSeatFinderToken);
    if (!url) {
      return "";
    }
    ...
    openUrl.searchParams.set(
      "returnTo",
      `/dashboard/${weddingId}/planner?tab=seating`,
    );
    return openUrl.toString();
  };

  const getLayoutFileName = (suffix: string) => {
    const safeName = layout.layout_name
      .replace(/[^a-z0-9]+/gi, "-")
      .replace(/^-|-$/g, "")
      .toLowerCase();
  ...
  const downloadSeatingLayoutImage = async () => {
    ...
    try {
      const ratio =
        layout.venue_shape === "square"
          ? 1
          : Math.max(
              0.35,
              layout.venue_width / Math.max(1, layout.venue_height),
            );
      ...
      const loaded = new Promise<void>((resolve, reject) => {
        ...
        image.onerror = () =>
          reject(new Error("Unable to render the layout image."));
      });
      image.src = svgUrl;
      await loaded;
      ...
      const isPhoneBrowser = /android|iphone|ipad|ipod|mobile/i.test(
        navigator.userAgent,
      );
      if (isPhoneBrowser) {
        window.setTimeout(() => {
        ...
        setChartStatus(
          "If the layout did not download, use the opened image and save it to your phone.",
        );
      } else {
      ...
      setChartStatus(
        err instanceof Error
          ? err.message
          : "Unable to download the seating layout image.",
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const stopDragging = () => {
      const draggedTable = tablesRef.current.find(
        (table) => table.id === draggingTable,
      );
  ...
  useEffect(() => {
    ...
    const moveObject = (event: PointerEvent) => {
      const position = getPointerPosition(event.clientX, event.clientY);
      if (!position) return;
      lastObjectDragPositionRef.current = {
    ...
    const stopDragging = () => {
      const draggedObject = objectsRef.current.find(
        (object) => object.id === draggingObject,
      );
  ...
  const resetTablePositions = () => {
    const resetTables = tables.map((table, index) => ({
      ...table,
      ...getDefaultTablePosition(index),
    }));
    setTables(resetTables);
    void Promise.all(
      resetTables.map((table) =>
  ...
  const unassignedGuests = useMemo(() => {
    return guests
      ...
      .filter((guest) => {
        ...
        const matchesGroup =
          groupFilter === "all" ? true : guest.guest_group === groupFilter;
  ...
  const seatingSummary = useMemo(() => {
    const totalGuests = guests.reduce(
      (total, guest) => total + getPartySize(guest),
      0,
    );
    const seatedGuests = assignments.reduce(
      (total, assignment) =>
        total + getPartySize(guestMap.get(assignment.rsvp_id)),
      0,
    );
  ...
  const selectedObjectData =
    (layout.layout_data?.objects || []).find(
      (object) => object.id === selectedObject,
    ) || null;
  const selectedTableSize = selectedTableData
    ? getTableSize(selectedTableData)
    : null;
      ...
      <div className="bg-white dark:bg-white/5 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 md:p-12 soft-shadow border border-border">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 sm:mb-10">
          <div>
            ...
            <p className="text-xs sm:text-sm text-text-secondary mt-1">
              Design your venue layout, drag tables into place, and assign
              confirmed guests.
            </p>
          </div>
          <button
            onClick={openAddTableModal}
            className="w-full md:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-primary text-white rounded-xl sm:rounded-2xl font-bold hover:shadow-lg transition-all active:scale-95 min-h-[44px]"
          >
          ...
          <div className="mb-6 flex flex-col gap-3 rounded-2xl border border-primary/20 bg-primary/5 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-bold text-foreground">
                Free seating sample:{" "}
                {Math.min(tables.length, FREE_PLAN_LIMITS.seatingTables)} /{" "}
                {FREE_PLAN_LIMITS.seatingTables} tables
              </p>
              <p className="mt-1 text-xs text-text-secondary">
                Upgrade for unlimited tables, guest check-in, seat links, and
                bulk seat emails.
              </p>
            </div>
            <UpgradeButton
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            [
              "Seated Guests",
              `${seatingSummary.seatedGuests}/${seatingSummary.totalGuests}`,
            ],
            ...
            <div
              key={label}
              className="rounded-2xl border border-border bg-neutral/50 p-4 text-center sm:text-left"
            >
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-text-secondary">
                {label}
              </p>
              <p
                className={`mt-2 font-serif text-2xl font-bold ${label === "Open Seats" && Number(value) < 0 ? "text-red-500" : "text-foreground"}`}
              >
                {value}
              </p>
            </div>
        ...
        <div className="mb-6 rounded-2xl border border-border bg-neutral/40 p-4">
          <div className="grid gap-3 lg:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_auto_auto_auto]">
            <label className="space-y-1">
              <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-text-secondary">
                Layout Name
              </span>
              <input
                value={layout.layout_name}
                onChange={(event) =>
                  updateLayout({ layout_name: event.target.value })
                }
                className="w-full rounded-xl border border-border bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-primary"
              />
            </label>
            <label className="space-y-1">
              <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-text-secondary">
              ...
              <select
                value={layout.venue_shape}
                onChange={(event) =>
                  updateLayout({
                    venue_shape: event.target.value as VenueShape,
                  })
                }
                className="w-full rounded-xl border border-border bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-primary"
              >
            ...
            <label className="space-y-1">
              ...
              <input
                ...
                min={30}
                max={240}
                value={layout.venue_width}
                onChange={(event) =>
                  updateLayout({ venue_width: Number(event.target.value) })
                }
                className="w-full rounded-xl border border-border bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-primary"
              />
            </label>
            <label className="space-y-1">
              ...
              <input
                ...
                min={30}
                max={240}
                value={layout.venue_height}
                disabled={layout.venue_shape === "square"}
                onChange={(event) =>
                  updateLayout({ venue_height: Number(event.target.value) })
                }
                className="w-full rounded-xl border border-border bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-primary disabled:opacity-50"
              />
            </label>
            <button
              type="button"
              onClick={() =>
                updateLayout({ grid_enabled: !layout.grid_enabled })
              }
              className={`inline-flex min-h-[44px] items-center justify-center rounded-xl border px-4 py-2 text-sm font-bold transition ${layout.grid_enabled ? "border-primary bg-primary text-white" : "border-border bg-white text-text-secondary hover:text-primary"}`}
            >
            ...
            <button
              ...
              onClick={() =>
                updateLayoutData({
                  showMeasurements: !layout.layout_data?.showMeasurements,
                })
              }
              className={`inline-flex min-h-[44px] items-center justify-center rounded-xl border px-4 py-2 text-sm font-bold transition ${layout.layout_data?.showMeasurements ? "border-primary bg-primary text-white" : "border-border bg-white text-text-secondary hover:text-primary"}`}
            >
            ...
            <button
              ...
              className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-border bg-white px-4 py-2 text-sm font-bold text-text-secondary transition hover:border-primary/30 hover:text-primary"
            >
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div className="min-w-0">
                ...
                <h3 className="mt-1 font-serif text-xl font-bold text-foreground">
                  Guest seat links and venue check-in
                </h3>
                <p className="mt-1 text-xs leading-5 text-text-secondary">
                  Generate guest codes, email QR seat links, or print one venue
                  QR for guests to find their seats.
                </p>
                {(seatFinderSummary.attendingCount !== undefined ||
                  ...
                  <p className="mt-2 text-xs font-semibold text-text-secondary">
                    {seatFinderSummary.attendingCount !== undefined && (
                      <>
                        Guests: {seatFinderSummary.attendingCount || 0} -
                        Assigned: {seatFinderSummary.assignedCount || 0} - Sent:{" "}
                        {seatFinderSummary.sentCount || 0} - Checked in:{" "}
                        {seatFinderSummary.checkedInCount || 0}
                      </>
                    )}
                    {seatFinderStatus && (
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                ...
                <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
                  <button
                    type="button"
                    onClick={() => void generateSeatLinks()}
                    disabled={seatFinderLoading !== null}
                    className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-primary/20 bg-white px-3 py-2 text-xs font-bold text-primary transition hover:bg-primary hover:text-white disabled:opacity-50"
                  >
                    {seatFinderLoading === "generate" ? (
                  ...
                  <button
                    type="button"
                    onClick={() => void sendSeatLinks(false)}
                    disabled={seatFinderLoading !== null}
                    className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-bold text-white transition hover:bg-primary-hover disabled:opacity-50"
                  >
                    {seatFinderLoading === "send" ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
                  ...
                  <button
                    type="button"
                    onClick={() => void sendSeatLinks(true)}
                    disabled={seatFinderLoading !== null}
                    className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-border bg-white px-3 py-2 text-xs font-bold text-text-secondary transition hover:border-primary/30 hover:text-primary disabled:opacity-50"
                  >
                    {seatFinderLoading === "resend" ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
                      <Mail className="h-4 w-4" />
                    )}
                  ...
                  <a
                    href={`/dashboard/${weddingId}/planner/check-in`}
                    className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-border bg-white px-3 py-2 text-xs font-bold text-text-secondary transition hover:border-primary/30 hover:text-primary"
                  >
                    <CheckCircle2 className="h-4 w-4" />
                    Check-In
                  </a>
                  <a
                    href={`/user-guide/seating-planner?returnTo=${encodeURIComponent(`/dashboard/${weddingId}/planner?tab=seating`)}`}
                    className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-primary/20 bg-white px-3 py-2 text-xs font-bold text-primary transition hover:bg-primary hover:text-white"
                  >
                ...
                <div className="mb-3 flex flex-col gap-3 rounded-2xl border border-border bg-neutral/40 p-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs font-semibold text-text-secondary">
                      {chartStatus ||
                        "Open the chart fullscreen or download the final floor plan as a PNG."}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center">
                    <button
                      type="button"
                      onClick={() => void downloadSeatingLayoutImage()}
                      className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl border border-border bg-white px-3 py-2 text-xs font-bold text-text-secondary transition hover:border-primary/30 hover:text-primary"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl bg-primary px-3 py-2 text-xs font-bold text-white transition hover:bg-primary-hover"
                    >
                      {isChartFullscreen ? (
                  ...
                  >
                    ...
                    {layout.layout_data?.showMeasurements && (
                      ...
                      >
                        ...
                        <text x="50" y="1.4" textAnchor="middle">
                          {layout.venue_width}&apos;
                        </text>
                            ...
                            <g>
                              ...
                              {layout.layout_data?.showMeasurements && (
                                <text
                                  ...
                                  fontWeight="800"
                                  fill="#4aaaf5"
                                  transform={`rotate(${object.rotation * -1} ${object.width / 2 + 2} 0)`}
                                >
                                  {object.width.toFixed(0)}&apos;
                                </text>
                              )}
                            </g>
                          )}
                        ...
                        >
                          {chairs.map((chair, index) => (
                            <rect
                              ...
                              stroke="#555"
                              strokeWidth="0.22"
                              transform={`rotate(${chair.rotation} ${chair.x} ${chair.y})`}
                              filter="url(#floor-shadow)"
                            />
                          ))}
                          ...
                          {tableAssignments.length > 0 && (
                            <text
                              ...
                              fill="#7A5A61"
                            >
                              {tableAssignments.length} parties
                            </text>
                          )}
                        </g>
                      );
                  ...
                  {tables.length === 0 && (
                    <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-text-secondary opacity-50">
                      ...
                      <p className="text-sm mt-2">
                        Click &quot;Add Table&quot; to start your floor plan.
                      </p>
                    </div>
                  )}
                </div>
                <div className="mt-4 grid grid-cols-1 md:grid-cols-3 items-center gap-4 rounded-2xl border border-border bg-neutral/40 p-4 min-h-[100px]">
                  ...
                  <div className="flex flex-col justify-center text-center md:text-left">
                    ...
                    <p className="mt-1 text-sm font-bold text-foreground">
                      {layout.layout_name} - {layout.venue_width}&apos; x{" "}
                      {layout.venue_height}&apos;
                    </p>
                      ...
                      <div className="relative flex items-center justify-center bg-white dark:bg-white/5 rounded-full p-2.5 shadow-md border border-border/60">
                        ...
                        <div className="grid grid-cols-3 grid-rows-3 gap-1.5 w-32 h-32 relative">
                          ...
                          <button
                            ...
                            onClick={
                              selectedTableData
                                ? () => nudgeSelectedTable(0, -5)
                                : () => nudgeSelectedObject(0, -5)
                            }
                          ...
                          <button
                            ...
                            onClick={
                              selectedTableData
                                ? () => nudgeSelectedTable(-5, 0)
                                : () => nudgeSelectedObject(-5, 0)
                            }
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (selectedTableData && selectedTableSize) {
                              const newRotation =
                                (selectedTableSize.rotation + 15) % 360;
                              void updateTableStyle(selectedTableData.id, {
                                rotation: newRotation,
                              });
                            } else if (selectedObjectData) {
                              const newRotation =
                                (selectedObjectData.rotation + 15) % 360;
                              updateVenueObject(selectedObjectData.id, {
                                rotation: newRotation,
                              });
                            }
                ...
                <div className="rounded-2xl border border-border bg-neutral/40 p-4">
                  <div>
                    <div>
                      ...
                      <p className="mt-1 text-xs font-semibold text-text-secondary">
                        {getSeatsUsed(selectedTableData.id)} of{" "}
                        {selectedTableData.capacity} seats used
                      </p>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => openEditTableModal(selectedTableData)}
                          className="inline-flex min-h-[40px] items-center justify-center gap-2 rounded-xl border border-border bg-white px-3 py-2 text-xs font-bold text-text-secondary transition hover:border-primary/30 hover:text-primary"
                        >
                          <Edit2 className="h-4 w-4" /> Edit
                        </button>
                        <button
                          type="button"
                          onClick={() => void deleteTable(selectedTableData.id)}
                          className="inline-flex min-h-[40px] items-center justify-center gap-2 rounded-xl border border-red-100 bg-white px-3 py-2 text-xs font-bold text-red-500 transition hover:bg-red-50"
                        >
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-2">
                        <label className="space-y-1">
                          <span className="block text-[9px] font-black uppercase tracking-[0.14em] text-text-secondary">
                            Width
                          </span>
                          <input
                            type="number"
                            min={7}
                            max={28}
                            value={selectedTableSize.width}
                            onChange={(event) =>
                              void updateTableStyle(selectedTableData.id, {
                                width: Number(event.target.value),
                              })
                            }
                            className="w-full rounded-xl border border-border bg-white px-3 py-2 text-xs font-bold outline-none focus:border-primary"
                          />
                        </label>
                        <label className="space-y-1">
                          <span className="block text-[9px] font-black uppercase tracking-[0.14em] text-text-secondary">
                          ...
                          <input
                            ...
                            min={6}
                            max={24}
                            value={selectedTableSize.height}
                            onChange={(event) =>
                              void updateTableStyle(selectedTableData.id, {
                        ...
                        <label className="space-y-1">
                          ...
                          <input
                            type="number"
                            step={15}
                            value={selectedTableSize.rotation}
                            onChange={(event) =>
                              void updateTableStyle(selectedTableData.id, {
                ...
                <div className="rounded-2xl border border-border bg-neutral/40 p-4">
                  <div>
                    <div className="min-w-0 flex-1">
                      ...
                      <input
                        value={selectedObjectData.label}
                        onChange={(event) =>
                          updateVenueObject(selectedObjectData.id, {
                            label: event.target.value,
                          })
                        }
                        className="mt-2 w-full rounded-xl border border-border bg-white px-3 py-2 text-sm font-bold outline-none focus:border-primary"
                      />
                      <div className="mt-4 grid grid-cols-3 gap-2">
                        <label className="space-y-1">
                          <span className="block text-[9px] font-black uppercase tracking-[0.14em] text-text-secondary">
                      ...
                      <button
                        type="button"
                        onClick={() => deleteVenueObject(selectedObjectData.id)}
                        className="mt-4 inline-flex min-h-[40px] items-center justify-center gap-2 rounded-xl border border-red-100 bg-white px-3 py-2 text-xs font-bold text-red-500 transition hover:bg-red-50"
                      >
                ...
                <div className="p-4 sm:p-6 border-b border-border bg-neutral/30 dark:bg-neutral/10">
                              ...
                              >
                                <span className="min-w-0">
                                  ...
                                  <span className="block text-[10px] font-semibold text-text-secondary">
                                    {getSeatLabel(assignment)} - party of{" "}
                                    {getPartySize(guest)}
                                  </span>
                                </span>
                                <button
                  ...
                  <div className="relative mb-3">
                    ...
                    <input
                      ...
                      className="icon-field-left w-full bg-white border border-border rounded-xl pl-14 pr-4 py-2.5 text-sm focus:outline-none focus:border-primary transition-colors"
                    />
                  </div>
                  <select
                    value={groupFilter}
                    onChange={(event) =>
                      setGroupFilter(event.target.value as "all" | GuestGroup)
                    }
                    className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-primary transition-colors"
                  >
                    ...
                    {unassignedGuests.length === 0 ? (
                        ...
                        {searchQuery || groupFilter !== "all" ? (
                          ...
                          <>
                            ...
                            <p className="text-sm text-text-secondary font-serif italic">
                              All confirmed guests have seats!
                            </p>
                          </>
                        )}
                      </motion.div>
                      ...
                      unassignedGuests.map((guest) => (
                        <motion.div
                          ...
                          key={guest.id}
                          className={`flex items-center justify-between p-4 rounded-2xl border transition-all group ${selectedTable ? "bg-white border-border hover:border-primary/50 hover:shadow-md" : "bg-neutral/50 border-transparent opacity-60 grayscale"}`}
                        >
                          ...
                          <button
                            ...
                            onClick={() =>
                              selectedTable &&
                              void assignGuest(guest, selectedTable)
                            }
                            className={`p-2.5 rounded-xl transition-all shrink-0 ${selectedTable ? "bg-primary/10 text-primary hover:bg-primary hover:text-white hover:scale-105 active:scale-95" : "bg-neutral text-border cursor-not-allowed"}`}
                            title={
                              selectedTable
                                ? "Assign to selected table"
        ...
        {isTableModalOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 bg-foreground/55 backdrop-blur-sm z-50"
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-serif font-bold text-xl sm:text-2xl text-foreground">
                  {editingTableId ? "Edit Table" : "Customize Table"}
                </h3>
                <button
                  onClick={() => setIsTableModalOpen(false)}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${tableFormData.shape === shape.id ? "border-primary bg-primary/5 text-primary" : "border-border hover:border-primary/30 text-text-secondary hover:bg-neutral"}`}
                      >
                        <shape.icon
                          className="w-6 h-6 mb-2"
                          strokeWidth={
                            tableFormData.shape === shape.id ? 2.5 : 1.5
                          }
                        />
                  ...
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      onClick={() =>
                        setTableFormData((current) => ({
                          ...current,
                          capacity: Math.max(1, current.capacity - 1),
                        }))
                      }
