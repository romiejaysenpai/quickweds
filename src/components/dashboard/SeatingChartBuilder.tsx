    ...
    return (
        <div className="space-y-6 relative">
            <div className="bg-white rounded-[2.5rem] p-6 sm:p-10 soft-shadow border border-border">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                    <div>
                        ...
                        <p className="text-sm text-text-secondary mt-1">Design your floor plan and keep every table in sync with the guest list.</p>
                    </div>
                    <button onClick={openAddTableModal} className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white rounded-2xl font-bold hover:shadow-lg transition-all active:scale-95">
                                        ...
                                        {tables.map((table) => {
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    className={`relative p-6 transition-all cursor-pointer bg-white group flex flex-col ${getShapeClasses(table.table_shape)} ${isSelected ? 'shadow-xl ring-2 ring-primary scale-[1.02] border-transparent' : 'border-2 border-border hover:border-primary/40 hover:shadow-md'}`}
                                                >
                                                    ...
                                                    <div className="flex-1 flex flex-col justify-center">
                                                        <div className="flex flex-wrap items-center justify-center gap-2">
                                                            {tableAssignments.map((assignment) => {
                                                                ...
                                                                return (
                                                                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} key={assignment.id} className="relative z-10 inline-flex flex-col gap-1 px-3 py-2 bg-neutral rounded-2xl text-xs font-medium border border-border group/seat">
                                                                        ...
                                                                        <button onClick={(event) => { event.stopPropagation(); void removeAssignment(assignment); }} className="text-text-secondary hover:text-red-500 hover:bg-red-50 p-0.5 rounded-full opacity-0 group-hover/seat:opacity-100 transition-all absolute -top-2 -right-2 bg-white shadow-sm border border-border">
                                                                            <X className="w-3 h-3" />
                                                                        </button>
                                                                    </motion.div>
                                                                );
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-border shadow-sm overflow-hidden flex flex-col h-full max-h-[700px]">
                            <div className="p-6 border-b border-border bg-neutral/30">
                                ...
                                <div className="relative mb-3">
                                    ...
                                    <input type="text" placeholder="Search guests..." value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="w-full bg-white border border-border rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-primary transition-colors" />
                                </div>
            ...
            <AnimatePresence>
                {isTableModalOpen && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40" onClick={() => setIsTableModalOpen(false)} />
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-[2.5rem] p-8 z-50 shadow-2xl border border-border">
